...
import { Mail, Lock, User, ArrowRight, Loader2, MessageSquare } from 'lucide-react';
import SEO from '../SEO';
...
export function CustomerLogin() {
  ...
  const { signInWithEmail, signInWithGoogle, signInWithFacebook, user } = useCustomerAuth();
      ...
      <div className="min-h-screen flex items-center justify-center pt-20 pb-12 px-6">
        <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 p-8 rounded-2xl">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            {success && <div className="p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm font-medium flex items-center justify-center gap-2"><Loader2 className="w-4 h-4 animate-spin"/> {success}</div>}
            ...
            <div className="flex justify-between items-center text-sm">
              <label className="flex items-center gap-2 text-zinc-400">
                <input type="checkbox" className="rounded bg-zinc-900 border-zinc-700 text-indigo-500" />
                Remember me
              </label>
              <Link to="/forgot-password" className="text-indigo-400 hover:text-indigo-300">Forgot password?</Link>
            </div>

            <button disabled={loading} type="submit" className="w-full flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold py-3 rounded-xl transition">
              {loading ? (
                <>
            ...
            <div className="grid grid-cols-2 gap-4">
               ...
               <button onClick={handleFacebook} type="button" className="flex items-center justify-center gap-2 py-2 px-4 border border-zinc-800 hover:bg-zinc-800 rounded-xl text-white text-sm transition bg-[#1877F2]/10 hover:bg-[#1877F2]/20 border-[#1877F2]/30">
